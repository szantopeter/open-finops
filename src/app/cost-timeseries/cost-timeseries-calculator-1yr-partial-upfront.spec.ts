    ...
    it('should calculate on 1yr partial upfront costs from start date to end of firstFullYear', () => {
      ...
      result.forEach((costTimeseries, riIndex) => {
        ...
        expectedCosts.forEach((expectedMonth, index) => {
          ...
          expect(actualMonth.cost.partialUpfront_1y).toBeDefined();
          expect((actualMonth.cost.partialUpfront_1y as any).upfrontCost).withContext('Upfront cost mismatch ri index: ' + riIndex + ' index: ' + index).toBe(expectedMonth.upfrontCost * count);
          expect((actualMonth.cost.partialUpfront_1y as any).monthlyCost).withContext('Monthly cost mismatch ri index: ' + riIndex + ' index: ' + index + ' Actual month: ' + actualMonth.year + '.' + actualMonth.month ) .toBe(expectedMonth.activeDays * onDemandPrice * riDiscount * count);

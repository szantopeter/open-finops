...
describe('RiMonthlySpendingService', () => {
  ...
  it('places full upfront exactly once (expiry month) for All Upfront scenarios and monthly payments are zero', async () => {
    ...
    const r = portfolio.rows[0] as any;
    ...
    const pricingResult = await pricingSvc.loadPricingForPaths([fileName]).toPromise?.() ?? await pricingSvc.loadPricingForPaths([fileName]).toPromise();
    const pricingRecords = (pricingResult as any).pricingRecords || [];
    ...
    // Call the service
    const result = await service.getMonthlyTablesForRiPortfolio(portfolio as any);
    ...
    expect(pricingRecords.length).toBeGreaterThan(0, 'Pricing records should be present for the test asset');
    // aggregationSvc is the same injectable used by the service; check for errors
    expect(aggregationSvc.lastErrors.unmatchedPricing.length).toBe(0, `Aggregation reported unmatched pricing: ${JSON.stringify(aggregationSvc.lastErrors.unmatchedPricing.slice(0,5))}`);
    expect(aggregationSvc.lastErrors.missingRates.length).toBe(0, `Aggregation reported missing rates: ${JSON.stringify(aggregationSvc.lastErrors.missingRates.slice(0,5))}`);
    expect(aggregationSvc.lastErrors.renewalErrors.length).toBe(0, `Aggregation reported renewal errors: ${JSON.stringify(aggregationSvc.lastErrors.renewalErrors.slice(0,5))}`);
    ...
    for (const s of allUpfront) {
      ...
      expect(upfrontMonths.length).toBe(1, `Expected exactly one upfront month for scenario ${s.scenario}`);
      expect(upfrontMonths[0]).toBe(expiryMonthKey, `Upfront month should be the RI expiry month for scenario ${s.scenario}`);
      ...
      for (const mk of months) {
        ...
        expect(totalRenewalRecurring).toBe(0, `Renewal recurring cost should be 0 for All Upfront scenario ${s.scenario} in month ${mk}`);
      }
    }
  });
});
